...
mscom_nll <- function(par, C_mat, nstocks, nyrs, r_priors, k_priors){
  ...
  # Check candidate r/k against priors
  # If candidate r/k values are outside bounds, report a large NLL
  # If candidate r/k values are inside bounds, proceed
  ri_check <- sum(ri < r_priors[,"r_lo"])==0 & sum(ri > r_priors[,"r_hi"])==0
  ki_check <- sum(ki < k_priors[,"k_lo"])==0 & sum(ki > k_priors[,"k_hi"])==0
  rk_check <- ri_check & ki_check
  # Outside bounds, blow up
  if(rk_check==F){
    nll_E <- 9999
...
mscom <- function(C_mat, years, stocks, res){
  ...
  optfit <- optim(par=start_vals_log, fn=mscom_nll,
                  ...
                  r_priors=r_priors, k_priors=k_priors,
