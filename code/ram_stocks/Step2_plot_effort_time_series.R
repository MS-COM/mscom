...
layout(matrix(data=c(1,2,
                     1,3,
                     1,4), ncol=2, byrow=T), width=c(0.6, 0.4))
...
# Plot correlatations
plot(vr_scaled ~ corr_avg, corr_df1, bty="n", las=1, bg="grey80", pch=21,
...
text(x=corr_ex$corr_avg, y=corr_ex$vr_scaled, labels=corr_ex$fishery, font=2, pos=2)
text(x=1, y=0, labels=paste0(nrow(corr_df1), " fisheries"), adj=1, cex=1.1)
lines(x=c(0,0), y=c(0,4), lty=3)
lines(x=c(-0.2,1), y=c(1,1), lty=3)

# New par
par(mar=c(3.5, 1.5, 1.0, 0.5), mgp=c(1,0.8,0), xpd=NA)
...
for(i in 1:length(ex_fisheries)){
  ...
  plot(erdata$year, erdata[,2], type="l", bty="n", 
       xlab="", ylab=ylabel, main=fishery,
       xaxt="n", yaxt="n", col=colors[1], lwd=1.1, cex.main=0.9, cex.lab=1.2,
       xlim=c(xmin, xmax), ylim=c(min(erdata[,2], na.rm=T), max(erdata[,2], na.rm=T)))
  ...
  for(j in 3:ncol(erdata)){
    par(new=T)
    plot(erdata$year, erdata[,j], type="l", bty="n", 
         xaxt="n", yaxt="n", xlab="", ylab="", col=colors[j-1], lwd=1.1,
         xlim=c(xmin, xmax), ylim=c(min(erdata[,j], na.rm=T), max(erdata[,j], na.rm=T)))
  }

  # Add legend
  # legend("topleft", bty="n", legend=stocks, col=colors, lwd=1.1)
